...
doc.xpath("//div[@data-component-term='tweet']").each do |tweet|
  # Tweet時間
  puts Time.at(tweet.xpath(".//a[@class='ProfileTweet-timestamp js-permalink js-nav js-tooltip']/span").first['data-time'].to_i)

  # Tweet本文
  puts tweet.xpath(".//p[@class='ProfileTweet-text js-tweet-text u-dir']").text

  # Retweet数
  retweet = tweet.xpath(".//li[@class='ProfileTweet-action ProfileTweet-action--retweet js-toggle-state js-toggle-rt']/button[@class='ProfileTweet-actionCount js-actionCount js-tooltip']")
  if !retweet.empty?
